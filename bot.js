...
async function translateGoogle(language) {
    var datasdsd = '';
    if (language) {
        datasdsd = await google.translate(language, {
            from: 'hi',
            to: 'en'
        });
    }
...
class BasicBot {
    ...
    async onTurn(context) {
        ...
        if (context.activity.type === ActivityTypes.Message) {
            ...
            let logdata = context.activity.text;

            if (/^[a-z]+[a-z0-9]*$/i.test(logdata) === false) {
                console.log(/^[a-z]+[a-z0-9]*$/i.test(logdata))
                logdata = await translateGoogle(logdata);

                context.activity.text = logdata.text;
            }
